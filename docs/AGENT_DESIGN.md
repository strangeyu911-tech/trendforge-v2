...
11 个 Agent，四段式拓扑。**主链路 1–10 固定顺序执行**（8 号 Editor 可回退 6 号 Writer，最多 2 轮）；11 号 FeedbackAnalyst 离线运行（消费数据回流后）。

```
SENSE     SignalScout → TrendAnalyst → AudienceInsight → AngleEditor
PRODUCE   Researcher → Writer ⇄ FactChecker → Editor (revise→Writer, ≤2轮)
...
| 5 | Researcher | brief, KB | evidences[]（ev_id/引文/来源/可信度/时间） | "改写 4 个 query，召回 10 条证据覆盖 5 来源" | 放宽过滤直取关键词检索 |
| 6 | Writer | brief, evidences | article（标题/正文[ev_xxx]/摘要） | "按 deep_dive 写成 900 字，引用 8/10 证据" | 基于大纲模板拼装（标注 is_fallback） |
| 7 | FactChecker | article, evidences | fact_check（论断数/有据率/未证实清单/置信度） | "核查 6 论断，5 有据，1 弱支持" | 引用计数启发式 |
| 8 | Editor | article, fact_check, 市场禁忌 | verdict（pass/revise/reject + Rubric 5 维评分 + 意见） | "质量 4.2/5，无合规命中，pass" | 规则合规扫描 + 默认 pass-with-warning |
| 9 | FormatAdapter | article, 形态清单 | formats{video_script, card, brief_news, comment} | "派生 4 形态，短视频脚本钩子=数据冲击型" | 模板截取（摘要=首段等） |
| 10 | Distributor | article, formats, 市场档案 | plan（平台×形态×受众×时段×理由） | "日本主发 LINE 摘要卡（通勤场景），X 发快讯" | 市场档案默认平台×全形态 |
| 11 | FeedbackAnalyst | contents + events | eval_report（质量/消费指标/问题/迭代建议） | "短视频脚本 CTR 高于图文 22%，建议 JP 市场加大视频权重" | 仅 SQL 统计无 LLM 建议 |
