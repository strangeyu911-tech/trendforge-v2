...
2. 否则并行拉取 HN + Dev.to + GDELT（best-effort）
...
async def fetch_market_signals(market_code: str, interests: dict | None = None,
    ...
    diag = {"hn": 0, "devto": 0, "gdelt": 0, "cache_hit": False, "degraded": False}
    ...
    hn, dev, gd = await safe_gather(
        fetch_hn(market_code), fetch_devto(market_code), fetch_gdelt(market_code))
    diag["hn"], diag["devto"], diag["gdelt"] = len(hn), len(dev), len(gd)

    merged = dedupe_merge(list(hn) + list(dev) + list(gd))
    if not merged:
        diag["degraded"] = True
        return [], diag
